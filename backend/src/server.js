...
if (!API_KEY) {
  ...
  console.error('You can generate a secure API key using: node scripts/generate-api-key.js');
  process.exit(1);
}
...
if (API_KEY === 'change-me-in-production' || API_KEY === 'your-secure-api-key-here-change-in-production') {
  console.error('❌ ERROR: API_KEY cannot use default/example values');
  console.error('Please generate a secure API key using: node scripts/generate-api-key.js');
  process.exit(1);
}
...
const validateAPIKey = (req, res, next) => {
  ...
  if (apiKey.length !== API_KEY.length || !crypto.timingSafeEqual(Buffer.from(apiKey), Buffer.from(API_KEY))) {
    return res.status(401).json({ 
      error: 'Unauthorized',
